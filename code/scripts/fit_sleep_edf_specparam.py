...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fit specparam fixed-mode aperiodic models to Sleep-EDF PSDs."
    )
    parser.add_argument(
        "--psd-npz",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional smoke-test limit after loading PSD.",
    )
...
def main() -> int:
    ...
    spectra = [
        psd_fit[epoch_idx, channel_idx]
        for epoch_idx in range(n_epochs)
        for channel_idx in range(n_channels)
    ]

    print(
        f"Fitting specparam fixed models: epochs={n_epochs}, channels={n_channels}, "
    ...
    ok = np.array([res["ok"] for res in results], dtype=bool).reshape(n_epochs, n_channels)
