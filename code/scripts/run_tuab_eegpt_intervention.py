...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fine-tune pretrained EEGPT on TUAB-format 10s referential windows "
...
def load_cache(args: argparse.Namespace):
    ...
    if args.cache_format == "npy":
        ...
        missing = [str(path) for path in paths.values() if path.name.endswith(".npy") and not path.exists()]
        if missing:
            raise FileNotFoundError(
                "Missing full-scale EEGPT/LaBraM cache files: " + ", ".join(missing)
            )
        return {
...
def load_eegpt_model(args: argparse.Namespace, n_chans: int, n_times: int, device):
    import torch
    ...
    load_summary = {
        ...
        "model_source": "braindecode.models.EEGPT with braindecode/eegpt-pretrained PyTorch checkpoint",
    }
...
def fit_eegpt(x, y, subjects, train_indices, args):
    import numpy as np
    import torch
    ...
    model, load_summary = load_eegpt_model(args, x.shape[1], x.shape[2], device)
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for start in range(0, len(val_idx), args.batch_size):
                ...
                xb = torch.from_numpy(np.array(x[batch_idx], dtype=np.float32, copy=True)).to(device)
                yb = torch.from_numpy(y[batch_idx].astype("float32")[:, None]).to(device)
                logits = model(xb)
                val_losses.append(float(criterion(logits, yb).item()) * int(len(batch_idx)))
...
def write_markdown(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# TUAB EEGPT Intervention Report",
        "",
        "EEGPT uses the Braindecode EEGPT implementation initialized from the",
        "`braindecode/eegpt-pretrained` PyTorch checkpoint, then fine-tuned on",
...
def main() -> int:
    ...
    if len(train_idx) == 0 or len(eval_idx) == 0:
        raise ValueError("Need both train and eval EEGPT windows.")

    model, threshold, train_log, train_meta = fit_eegpt(x, y, subjects, train_idx, args)
    prediction_rows = []
    eval_rows = []
    ...
    for start in range(0, len(eval_idx), args.intervention_batch_size):
        ...
        x_batch = np.array(x[batch_idx], dtype=np.float32, copy=True)
    ...
    write_csv(args.output_dir / "tuab_eegpt_intervention_eval_metrics.csv", eval_rows)
    write_csv(args.output_dir / "tuab_eegpt_intervention_predictions.csv", prediction_rows)
    write_csv(args.output_dir / "tuab_eegpt_intervention_subject_bootstrap.csv", boot)
    write_csv(args.output_dir / "tuab_eegpt_training_log.csv", train_log)
    write_markdown(args.output_dir / "tuab_eegpt_intervention_subject_bootstrap.md", boot)
    ...
    metadata = {
        ...
        "normalization": "official EEGPT/LaBraM TUAB-style uV windows; no per-window amplitude normalization",
        "preprocessing": "23 referential TUAB channels, 0.1-75 Hz bandpass, 50 Hz notch, 200 Hz resample, 10s windows",
        "intervention": "phase-preserving FFT amplitude edit after EEGPT preprocessing and before model forward",
    ...
    (args.output_dir / "tuab_eegpt_intervention_metadata.json").write_text(
