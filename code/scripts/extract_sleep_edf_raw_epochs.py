...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract raw Sleep-EDF EEG epochs into a reusable NumPy cache."
    )
    parser.add_argument(
        "--epochs-csv",
        type=Path,
    ...
    parser.add_argument(
        "--compressed",
        action="store_true",
        help="Use compressed NPZ output. Slower, but smaller.",
    )
...
def main() -> int:
    ...
    for recording, recording_rows in rows_by_recording.items():
        ...
        print(f"{recording}: raw epoch shape {recording_array.shape}", flush=True)
