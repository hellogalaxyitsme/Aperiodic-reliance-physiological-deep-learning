...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run PhysioNet MI PSD intervention and train-on-representation controls "
            "with subject-held-out folds and subject bootstrap intervals."
        )
    )
    parser.add_argument(
        "--index-csv",
...
def encode_labels(labels):
    ...
    return y, encoder.classes_.tolist()
...
def append_subject_rows(rows, base_row, subjects, y_true, y_pred, n_classes):
    ...
    for subject in sorted(np.unique(subjects)):
        ...
        support = {
            str(cls): int((y_true[mask] == cls).sum())
            for cls in range(n_classes)
        }
        rows.append(
            {
...
def subject_bootstrap(subject_rows, n_bootstrap: int, ci: float, seed: int):
    ...
    direct_groups = list(df.groupby(["task", "train_input", "test_input", "classes"], sort=False))
    for (task, train_input, test_input, classes), group in direct_groups:
        for metric in METRICS:
            subject_values = (
                group.groupby("subject", sort=False)[metric]
                .mean()
    ...
    for task, group in full_train.groupby("task", sort=False):
...
def write_markdown(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# PhysioNet MI PSD Intervention Report",
        "",
        "Subject-bootstrap balanced accuracy over held-out subjects.",
        "",
        "| " + " | ".join(focus.columns) + " |",
...
def main() -> int:
    ...
    arrays = {
        "full_log_psd": decomp["log_psd"][:n_trials],
        "aperiodic_spectrum": decomp["aperiodic_log_psd"][:n_trials],
        "flattened_log_psd": decomp["log_psd"][:n_trials] - decomp["aperiodic_log_psd"][:n_trials],
    }
    features = {
    ...
    folds = make_group_folds(groups, args.n_splits)

    fold_rows: list[dict[str, object]] = []
    subject_rows: list[dict[str, object]] = []
    for train_input in TRAIN_INPUTS:
        train_features = features[train_input]
        for fold_idx, (train_idx, test_idx) in enumerate(folds):
            ...
            print(f"{train_input} fold {fold_idx} done", flush=True)
    ...
    write_csv(args.output_dir / "psd_intervention_fold_metrics.csv", fold_rows)
    write_csv(args.output_dir / "psd_intervention_subject_metrics.csv", subject_rows)
    write_csv(args.output_dir / "psd_intervention_summary_metrics.csv", summary_rows)
    write_csv(args.output_dir / "psd_intervention_subject_bootstrap.csv", bootstrap_rows)
    write_markdown(args.output_dir / "psd_intervention_subject_bootstrap.md", bootstrap_rows)
    ...
    (args.output_dir / "psd_intervention_metadata.json").write_text(json.dumps(metadata, indent=2))
